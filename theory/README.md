...
| 06c | `06c_navier_stokes.md` | **V3 Stage C (theory)**: full incompressible Navier–Stokes = self-advecting Chorin projection on the verified operators; only nonlinear `C(u)u` is new. Yardstick shifts to nonlinear-CFD suite (Taylor–Green decay `4νk²`, MMS, energy conservation, convergence). Honest: equal-order P1 not inf–sup stable → incompressibility convergent residual, not machine | planned: `fluid_navier_stokes_verify.py` | theory only |
