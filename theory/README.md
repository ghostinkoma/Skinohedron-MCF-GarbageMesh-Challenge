...
| 06c | `06c_navier_stokes.md` | **V3 Stage C (verified)**: full incompressible Navier–Stokes = self-advecting Chorin projection. **Key finding**: the consistent `(B,Bᵀ)` projection from the one per-tet `G` cancels the apparent inf–sup limit — velocity divergence-free to **machine precision** (`‖Bu‖~1e-16`) through the nonlinear flow & all resolutions; Taylor–Green decay `4νk²` (3.1%, convergent); inviscid energy drift 0.000%. Honest: inf–sup's cost remains only in the **pressure** (corr ~0.24) | `fluid_navier_stokes_verify.py` | done |
